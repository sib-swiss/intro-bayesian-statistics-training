...
pp <- ggplot() + theme_bw() + theme(text = element_text(size = 18)) +
    geom_area(data = df2, mapping = aes(x = xs, y = ys), colour = "dodgerblue", size = 1, fill = "dodgerblue", alpha = 0.33) +
    geom_vline(xintercept = 0, colour = "dodgerblue", linetype = 1, size = 1) +
    geom_vline(xintercept = local_t, colour = "darkorchid4", linetype = 2, size = 2) +
    xlab("t") + ylab("density") + ylim(0, 1.25*yscale)

  pp <- pp + geom_area(data = subset(df2, xs > abs(local_t)), mapping = aes(x = xs, y = ys), colour = "darkorchid4", size = 1, fill = "darkorchid4", alpha = 0.33)
  pp <- pp + geom_area(data = subset(df2, xs < -abs(local_t)), mapping = aes(x = xs, y = ys), colour = "darkorchid4", size = 1, fill = "darkorchid4", alpha = 0.33)
  pp <- pp + geom_vline(xintercept = -local_t, colour = "darkorchid4", linetype = 3, size = 1)
