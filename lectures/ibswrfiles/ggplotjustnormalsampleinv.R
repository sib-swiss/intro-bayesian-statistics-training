...
pp <- ggplot() + theme_bw() + theme(text = element_text(size = 18)) +
    geom_area(data = df2, mapping = aes(x = xs,y = ys), colour = "dodgerblue", size = 1,
...
if (internalFlag) {
  pp <- pp + geom_area(data = subset(df2, xs > abs(local_mean)), mapping = aes(x = xs, y = ys), colour = "darkorchid4", size = 1, fill = "darkorchid4", alpha = 0.33)
  pp <- pp + geom_area(data = subset(df2, xs < -abs(local_mean)), mapping = aes(x = xs, y = ys), colour = "darkorchid4", size = 1, fill = "darkorchid4", alpha = 0.33)
  pp <- pp + geom_vline(xintercept = -local_mean, colour = "darkorchid4", linetype = 3, size = 1)
